...
| `render_visible_composite` | Visible composite PNG + mapping for vision; dual-delivery TextContent + ImageContent + optional `filesystem_path` (do not assume model saw the image). **Default max edge 1024** when max dims omitted (hard 4096); prefer region-first detail. Advanced surface aliases of this path share the same budget policy. |
